...
pub async fn set_infobar_image(event: PayloadEvent<SetInfobarImagePayload>) -> Result<(), anyhow::Error> {
	...
	let overlay_id = crate::infobar_overlay::push_overlay(&p.device, p.position, p.image, p.priority, p.duration_ms);
...
pub async fn clear_infobar_overlay(event: PayloadEvent<ClearInfobarOverlayPayload>) -> Result<(), anyhow::Error> {
	let p = event.payload;
	crate::infobar_overlay::clear_position_overlays(p.device, p.position).await;
	Ok(())
}
